...
string format_arg(const char* formatStr, va_list argList)
{
    std::string result;
    ...
    if (len > 0)
    {
        result.resize(len);
        va_copy(argListCopy, argList);
        const int bytesPrinted = azvsnprintf((char*)result.data(), result.size() + 1, formatStr, argListCopy);
        va_end(argListCopy);
...
string format_string(const char* formatStr, ...)
{
    ...
    string result = format_arg(formatStr, args);
    va_end(args);
    return result;
}
