...
#   define BehaviorConstant(value) []() { return value; }
    ...
    public:
        ...
        bool Set(FieldType ClassType::* member, BehaviorClass* currentClass, BehaviorContext* context);

        template<class FieldType>
        bool SetValue(BehaviorObject instance, const FieldType& value) {
            if (m_setter != nullptr) {
                return m_setter->Invoke(instance, value);
            }
            else if (m_dataSize > 0){
//                if (sizeof(FieldType) != m_dataSize) {
//                    return false;
//                }

                if constexpr (AZStd::is_pod_v<FieldType>) {
                    memcpy(reinterpret_cast<unsigned char*>(instance.m_address) + m_offset, &value, sizeof(FieldType));
                }
                else {
                    *reinterpret_cast<FieldType*>(reinterpret_cast<unsigned char*>(instance.m_address) + m_offset) = value;
                }
                return true;
            }
            return false;
        ...
        template<class FieldType>
        bool GetValue(BehaviorObject instance, FieldType& value) {
            if (m_getter != nullptr) {
                return m_getter->InvokeResult(value, instance);
            }
            else if (m_dataSize > 0) {
                if constexpr (AZStd::is_pod_v<FieldType>) {
                    memcpy(&value, reinterpret_cast<unsigned char*>(instance.m_address) + m_offset, sizeof(FieldType));
                }
                else {
                    value = *reinterpret_cast<FieldType*>(reinterpret_cast<unsigned char*>(instance.m_address) + m_offset);
                }
                return true;
            }
...
//        }
        ...
        struct DefaultSystemAllocator
        {
            ...
            {
                (void)userData;
//cjh                return azmalloc(sizeof(T), AZStd::alignment_of<T>::value, AZ::SystemAllocator, AZ::AzTypeInfo<T>::Name());
            ...
            {
                (void)userData;
//cjh                azfree(address, AZ::SystemAllocator, sizeof(T), AZStd::alignment_of<T>::value);
                free(address);
            }
        };
        ...
        template <class T, typename AZStd::enable_if<AZStd::HasherInvocable_v<T>>::type* = nullptr>
    ...
    public:
        ...
        struct ClassBuilder : public AZ::Internal::GenericAttributes<ClassBuilder<C>>
        {
            ...
            */
            ...
            template<class FieldType>
            ClassBuilder* Property(const char* name, FieldType C::* address);

            /// All enums are treated as the enum type
            template<AZStd::size_t Value, typename T = decltype(Value)>
...
#define AZ_BEHAVIOR_EBUS_EVENT_FUNCTION_TYPE_I(memberName) decltype(&ThisType::memberName)
    ...
    struct SetResult
    {
        ...
        static bool Set(BehaviorValueParameter& param, T&& result, bool IsValueCopy)
        {
            ...
            else if (IsValueCopy)
            {
                ...
                {
                    // value copy
                    if /* cjh constexpr */ (IsCopyAssignable<Type&, T>)
                    {
    ...
    template<class ClassType, class FieldType>
    bool BehaviorProperty::Set(FieldType ClassType::* member, BehaviorClass* currentClass, BehaviorContext* context)
    {
        m_offset = reinterpret_cast<size_t>(&(reinterpret_cast<ClassType const volatile*>(0)->*member));
        m_dataSize = sizeof(FieldType);
...
//cjh            SetClassDefaultAllocator<T>(behaviorClass, typename HasAZClassAllocator<T>::type());
...
//cjh                BehaviorContextBus::Event(this, &BehaviorContextBus::Events::OnRemoveGlobalMethod, name, globalMethodIt->second);
    ...
    template<class C>
    template<class FieldType>
    BehaviorContext::ClassBuilder<C>* BehaviorContext::ClassBuilder<C>::Property(const char* name, FieldType C::* member)
    {
        if (m_class)
        {
            BehaviorProperty* prop = aznew BehaviorProperty(Base::m_context);
            prop->m_name = name;
            if (!prop->Set(member, m_class, Base::m_context))
            {
                delete prop;
                return this;
            }
    ...
    //////////////////////////////////////////////////////////////////////////
    template<class C>
    template<AZStd::size_t Value, typename T>
...
//cjh            EBUS_EVENT_ID(((void*)(*arguments[0].GetAsUnsafe<C*>())), BehaviorObjectSignals, OnMemberMethodCalled, this);
        ...
        bool BehaviorEBusEvent<EBus, EventType, R(C::*)(Args...)>::Call(BehaviorValueParameter* arguments, unsigned int numArguments, BehaviorValueParameter* result) const
        {
            ...
            if /* cjh constexpr */ (s_isBusIdParameter)
            {
        ...
        void SetFunctionParameters<R(Args...)>::Set(AZStd::vector<BehaviorParameter>& params)
        {
            ...
            if /* cjh constexpr */ (sizeof...(Args) > 0)
            {
