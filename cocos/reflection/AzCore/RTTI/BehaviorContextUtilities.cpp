...
//cjh        if (Attribute* attribute = FindAttribute(ScriptCanvasAttributes::ExplicitOverloadCrc, method.m_attributes))
//        {
...
//        }
    ...
    {
        if (AZStd::string_ends_with(name, k_PropertyNameGetterSuffix))
        {
    ...
    {
        if (AZStd::string_ends_with(name, k_PropertyNameSetterSuffix))
        {
    ...
    AZStd::string ReplaceCppArtifacts(AZStd::string_view sourceName)
    {
        ...
        AZStd::string newName{sourceName.data(), sourceName.size()};
