...
//cjh #include <AzCore/NativeUI/NativeUIRequests.h>
//#include <AzCore/Debug/TraceMessageBus.h>
...
namespace AZ::Debug
{
    ...
    namespace Platform
    {
        ...
        void DebugBreak();
#endif
    void Terminate(int exitCode) {} //cjh
    }

//cjh    namespace DebugInternal
//    {
...
//cjh    static constexpr auto PrintfEventId = EventNameHash("Printf");
...
//
//cjh    AZ_CVAR_SCOPED(int, bg_traceLogLevel, DefaultLogLevel, nullptr, ConsoleFunctorFlags::Null, "Enable trace message logging in release mode.  0=disabled, 1=errors, 2=warnings, 3=info.");
//    AZ_CVAR_SCOPED(bool, bg_alwaysShowCallstack, false, nullptr, ConsoleFunctorFlags::Null, "Force stack trace output without allowing ebus interception.");
    ...
    void Trace::Init()
    {
//cjh        // if out ignored assert list exists, then another module has init the system
...
//        }
    ...
    {
        return nullptr;//cjh g_exceptionInfo;
    }
    ...
    {
        return 0;//
//cjh        auto val = AZ::Environment::FindVariable<int>(assertVerbosityUID);
//        if (val)
//        {
//            return val.Get();
//        }
